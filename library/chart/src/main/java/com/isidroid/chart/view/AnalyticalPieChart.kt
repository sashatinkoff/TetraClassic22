package com.isidroid.chart.view
...
import com.isidroid.chart.ext.dp
import com.isidroid.chart.ext.draw
import com.isidroid.chart.ext.sp
import com.isidroid.chart.model.AnalyticalPieChartModel
import com.isidroid.chart.model.AnalyticalPieChartState
import com.isidroid.chart.R as ChartRes
...
class AnalyticalPieChart @JvmOverloads constructor(
    ...
    private var marginTextFirst: Float = DEFAULT_MARGIN_TEXT_1.dp
    private var marginTextSecond: Float = DEFAULT_MARGIN_TEXT_2.dp
    private var marginTextThird: Float = DEFAULT_MARGIN_TEXT_3.dp
    private var marginSmallCircle: Float = DEFAULT_MARGIN_SMALL_CIRCLE.dp
    ...
    private var textCircleRadius: Float = 4.dp
    ...
    init {
        ...
        var textNumberSize: Float = 20.sp
        ...
        if (attrs != null) {
            val typeArray = context.obtainStyledAttributes(attrs, ChartRes.styleable.AnalyticalPieChart)

            // Секция списка цветов
            val colorResId = typeArray.getResourceId(ChartRes.styleable.AnalyticalPieChart_pieChartColors, 0)
            pieChartColors = typeArray.resources.getStringArray(colorResId).toList()

            // Секция отступов
            marginTextFirst = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartMarginTextFirst, marginTextFirst)
            marginTextSecond = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartMarginTextSecond, marginTextSecond)
            marginTextThird = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartMarginTextThird, marginTextThird)
            marginSmallCircle = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartMarginSmallCircle, marginSmallCircle)

            // Секция круговой диаграммы
            circleStrokeWidth = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartCircleStrokeWidth, circleStrokeWidth)
            circlePadding = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartCirclePadding, circlePadding)
            circlePaintRoundSize = typeArray.getBoolean(ChartRes.styleable.AnalyticalPieChart_pieChartCirclePaintRoundSize, circlePaintRoundSize)
            circleSectionSpace = typeArray.getFloat(ChartRes.styleable.AnalyticalPieChart_pieChartCircleSectionSpace, circleSectionSpace)

            // Секция текста
            textCircleRadius = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartTextCircleRadius, textCircleRadius)
            textAmountSize = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartTextAmountSize, textAmountSize)
            textNumberSize = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartTextNumberSize, textNumberSize)
            textDescriptionSize = typeArray.getDimension(ChartRes.styleable.AnalyticalPieChart_pieChartTextDescriptionSize, textDescriptionSize)
            textAmountColor = typeArray.getColor(ChartRes.styleable.AnalyticalPieChart_pieChartTextAmountColor, textAmountColor)
            textNumberColor = typeArray.getColor(ChartRes.styleable.AnalyticalPieChart_pieChartTextNumberColor, textNumberColor)
            textDescriptionColor = typeArray.getColor(ChartRes.styleable.AnalyticalPieChart_pieChartTextDescriptionColor, textDescriptionColor)
            textAmountStr = typeArray.getString(ChartRes.styleable.AnalyticalPieChart_pieChartTextAmount) ?: ""
    ...
    private fun resolveDefaultSize(spec: Int, defValue: Int): Int {
        return when (MeasureSpec.getMode(spec)) {
            MeasureSpec.UNSPECIFIED -> defValue.dp.toInt()
